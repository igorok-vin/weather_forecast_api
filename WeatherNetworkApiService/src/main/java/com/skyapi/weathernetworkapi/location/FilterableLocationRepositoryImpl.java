...
public class FilterableLocationRepositoryImpl implements FilterableLocationRepository {
    ...
    public Page<Location> listWithFilter(Pageable pageable, Map<String, Object> filterFields) {
        ...
        entityQuery.select(builder.construct(Location.class,entityRoot.get("code"),entityRoot.get("cityName"),entityRoot.get("regionName"),entityRoot.get("countryName"),entityRoot.get("countryCode"), entityRoot.get("enabled")));
        ...
        pageable.getSort().stream().forEach(order -> {
            ...
            if(order.isAscending()) {
                orderList.add(builder.asc(entityRoot.get(order.getProperty())));
            } else {
                orderList.add(builder.desc(entityRoot.get(order.getProperty())));
            }
        });
